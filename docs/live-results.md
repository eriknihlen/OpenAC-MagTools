...
| 5 | Pack Inventory hotkey (Ctrl+P) | r2 and r3: `+Acdream.AutoPack.utl` (KeepUpTo 1 on `Prismatic Taper`, pack 1) in place, Ctrl+P delivered from a second PowerShell process via `AppActivate` + `SendKeys` -- first by window title, then (r3) by the client's process id, logged as `pid=4716 title='acdream 0.1.10 | 217 fps | ...'`, `AppActivate(pid)=True`, `sent ctrl+p`. r4/r5: `/mt autopack` as the non-hotkey control | **The hotkey never fired**: neither session printed `Auto Pack - Started.` and the host log has no hotkey activity (b02/b03, c01/c02). The chord itself is not obviously blocked -- the client's own `P` binding is `SelectionPreviousSelection` with no modifier and `AppHotkeyRegistry.CollidesWithClient` is modifier-aware. The `/mt autopack` control DOES work: `<{Mag-Tools}>: Auto Pack - Started.` then `Auto Pack - Completed.` (d01/d02, e01/e02). **No item moved**: the main-pack grid is pixel-identical before and after in both runs. r5 tried to remove the ambiguity by minting a fresh loose taper first, but `@ci prismatictaper` answers `prismatictaper is not a valid weenie`, so whether the profile-matched items were simply already in their assigned pack could not be settled | **PENDING, narrowed (P12)** -- a temporary `IPluginHotkeyRegistration.IsBound`/`EffectiveChord` probe logged at `SessionReady` (well after the graphical host's keyboard source and input dispatcher are published) confirmed the default Ctrl+P chord DOES resolve and bind: `IsBound=True EffectiveChord=PluginKeyChord { Key = P, Ctrl = True, Alt = False, Shift = False }`. `AppHotkeyRegistry`'s collision check (`src/AcDream.App/Input/AppHotkeyRegistry.cs:148` `CollidesWithClient`) and modifier match (`:200-204` in `OnKeyDown`) both read correctly against the retail keymap (`P` alone is `SelectionPreviousSelection`, no Ctrl+P binding exists) and were verified live to be genuinely bound, not merely queued. `InventoryPacker.Start()` (`src/OpenAC.MagTools/Macros/InventoryPacker.cs:143`) is the exact same call the working `/mt autopack` control uses, so the packer's own no-op-without-a-profile guard is not the cause either. That leaves the raw keyboard-event delivery for a *physical* Ctrl+P press as the only unverified link, which cannot be exercised from the UI-probe route script (no raw-keypress verb) or reproduced without a live keyboard -- **host-side finding for the owner, no plugin-side fix to make**; see the defect-16 writeup below |
...
- **Defect 15 (CLOSED, P12) -- id data captured in one session was lost on
  the next session's dump.** After r1 the file held 15 `HasIdData=true`
  records; after r2 it held 2, with all 13 lost records still present under
  the identical `Id`+`ObjectClass` key and downgraded to an empty `false`
  record. Root cause found from the live-r5 `Inventory.after-r1.xml` ->
  `Inventory.after-r2.xml` diff: every downgraded record's on-disk
  `ObjectClass` was IDENTICAL in both files, and the downgraded shape
  (`LastIdTime=0`, empty `IntValues`) is exactly
  `MyWorldObjectRecord.CreateUnresolved`'s output written directly, not
  through `Combine` -- proving the previous-record lookup (`Id`+`ObjectClass`
  match) missed despite the final `ObjectClass` values agreeing. The lookup
  ran at the exact instant `Start()`'s synchronous `CaptureOwnedItems()`
  executes at login, while the server's `CreateObject` burst for owned/pack
  items can still be landing -- the item's *live* classification briefly
  reads as `Unknown` before it settles a moment later, so the very first
  dump's `previous.FirstOrDefault(prev => prev.Id == X.Id && prev.ObjectClass
  == X.ObjectClass)` (`InventoryLogger.cs:835` and `:849`, pre-fix) found
  nothing, wrote the bare unresolved stub, and that stub became the new
  `previous` baked in for the rest of the session. Fixed by matching on `Id`
  alone at both lookup sites: the server-assigned `Id` is the real identity,
  and `ObjectClass` is a derived, mutable description field --
  `MyWorldObjectRecord.Combine`'s own contract already keeps `older`'s
  `ObjectClass` once a match is found, so gating the match on it too was
  never load-bearing, only a source of false negatives. Commit `ce33e1f`;
  two new fail-first tests in `InventoryLoggerTests.cs`
  (`APreviouslyAppraisedItemResolvedButNotYetReAppraisedThisSessionKeepsItsIdData`,
  `APreviouslyAppraisedItemWithATransientlyDifferentLiveObjectClassKeepsItsIdData`
  -- the second failed on the pre-fix code, reproducing the exact
  live-evidence shape). Live-verified: three consecutive P12 sessions against
  the live-r5 character held `HasIdData=true` steady at 2/2/2 (no drop),
  confirming the fix holds outside the unit-test fakes too.
- **Defect 16 (NARROWED, P12; still open, host-side) -- the Pack Inventory
  hotkey does not fire.** Two deliveries of Ctrl+P (the second targeted at
  the client's own process id and confirmed sent by the sender log) produced
  no `Auto Pack - Started.`, while `/mt autopack` in the same build does. P12
  added a temporary `IPluginHotkeyRegistration.IsBound`/`EffectiveChord` log
  line at `OnSessionReady` (well after the graphical host's keyboard source
  and input dispatcher publish, per `docs/plugin-api.md`'s "queued until the
  input layer comes up" note) and confirmed live: `IsBound=True
  EffectiveChord=PluginKeyChord { Key = P, Ctrl = True, Alt = False, Shift =
  False }`. So the default chord DOES resolve and bind -- it is not refused
  by `AppHotkeyRegistry.CollidesWithClient`
  (`src/AcDream.App/Input/AppHotkeyRegistry.cs:148`; the retail keymap's only
  `DIK_P` binding is unmodified `P` -> `SelectionPreviousSelection`, not
  Ctrl+P) or `CollidesWithAnotherPlugin` (`:132`; only One Touch Heal's empty
  default chord is registered alongside it). `OnKeyDown`'s modifier match
  (`:200-204`) reads correctly: it requires the pressed modifiers to match
  the chord's `Ctrl`/`Alt`/`Shift` flags exactly, and the chat-focus carve-out
  at `:206` does not apply because the chord has `Ctrl=true`. Ctrl+J (an
  actual override) is proven to fire through this identical dispatch path in
  round 3, so nothing about the path is Ctrl-modifier-specific or
  key-specific either. `InventoryPacker.Start()`
  (`src/OpenAC.MagTools/Macros/InventoryPacker.cs:143`) is the exact same
  call `/mt autopack` makes, including its silent no-op-without-a-profile
  guard, so the packer's own logic is not the cause -- the working control
  command proves a profile exists and the call chain runs end to end. That
  leaves only the raw keyboard event delivery for a *physical* Ctrl+P key
  press (upstream of `AppHotkeyRegistry.OnKeyDown`, in
  `SilkKeyboardSource`/the GLFW binding) as the remaining unverified link;
  it cannot be exercised from the UI-probe route script (no raw-keypress
  verb) and needs either a live keyboard or a host-side trace to settle. No
  plugin-side workaround was applied -- this is a host-owner finding, not a
  plugin defect.
...
| Hotkey: Pack Inventory (Ctrl+P) | P12 narrowed this to the host: a live `IsBound`/`EffectiveChord` probe confirmed the default Ctrl+P chord resolves and binds (`IsBound=True`), collision checks and the dispatch/modifier-match code all read correctly, and `InventoryPacker.Start()` is the exact call the working `/mt autopack` control makes. Only the raw physical-key delivery upstream of `AppHotkeyRegistry.OnKeyDown` remains unverified -- needs a live keyboard or a host-side trace | PENDING (host-side) |
| Inventory logger: id data that survives a session | **CLOSED (P12)**: the previous-record match required both `Id` and `ObjectClass`, and the live object's classification can transiently read `Unknown` at the exact instant of the startup capture, so the match silently missed and downgraded good data. Fixed by matching on `Id` alone (commit `ce33e1f`); live-verified over three consecutive sessions with `HasIdData=true` held steady at 2/2/2, no drop. Throughput (15 of 60 ident-worthy items in one 4.5-minute session) is unchanged and not addressed by this fix | CLOSED |
